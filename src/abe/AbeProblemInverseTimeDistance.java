...
public class AbeProblemInverseTimeDistance {
	
	/**
	 * Note that by default the algorithm minimizes total costs and
	 * total costs = sumOfFixed + sumOfTransportCosts
	 * sumOfTransportCosts = sumOf(costPerTime*transportTime + costPerDistance*transportDistance)
	 * 
	 * @param args
	 * @throws IOException
	 */
...
//		VehicleRoutingTransportCostsMatrix matrix = matrixBuilder.build();
		...
		VehicleRoutingTransportCosts costs = new AbstractForwardVehicleRoutingTransportCosts() {
			...
			public double getTransportTime(String fromId, String toId, double departureTime, Driver driver, Vehicle vehicle) {
				//just a mean to 'inverse' transport times such that the longest relations become the fastest and the other way around
				return 4000. - getDistance(fromId, toId);
			}
